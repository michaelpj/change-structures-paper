\documentclass[manuscript,acmsmall,natbib=true]{acmart}
\pdfoutput=1
...
\email{luke.ong@cs.ox.ac.uk}          %% \email is recommended
...
\keywords{incremental computation, semantics, dcpos, Datalog}  %% \keywords are mandatory in final camera-ready submission
...
\begin{abstract}
 Incremental computation has recently been studied using the concepts of \emph{change
  ...
  functions, with particular application to generalised Datalog programs.
...
Consider the following classic Datalog program\footnote{See \citep[][part D]{abiteboul1995foundations} for an introduction to Datalog.},
...
evaluation\citep[See][section 13.1]{abiteboul1995foundations}, where we transform 
...
existential quantification, negation, and aggregation.\footnote{See, for
  example, \citep[LogiQL]{logicbloxWebsite,halpin2014logiql},
  \citep[Datomic]{datomicWebsite},
  \citep[Souffle]{souffleWebsite,scholz2016fast}, and
  \citep[DES]{saenz2011deductive}, which between them have all of these
  features and more. } 
...
\citep{semmleWebsite,avgustinov2016ql,sereni2008adding,schafer2010type}.
...
recursive aggregates \citep{demoor2013aggregates}, and other non-standard
...
\citep{gupta1993maintaining}, but these break down when the language is extended.
...
\emph{derivative} of that rule \citep{bancilhon1986amateur,bancilhon1986naive}[section
3.2.2]. The idea of performing incremental computation using derivatives has been
studied recently by \citet{cai2014changes}, who give an account using
...
typical when working with fixpoints \citep[See e.g.][]{compton1994stratified}) that treats a Datalog
...
to the precise derivative, and a soundness condition, appears in \citet{bancilhon1986amateur}.
...
the recursive functions defined in \cref{fig:datalogDerivatives}. This
derivative is composed of an upward part $\updiff(treeP(x))$ and a downward part
$\downdiff(treeP(x))$.
...
\begin{align*}
  ...
  &= p(x) \wedge \exists y. (child(x, y)\\
  & \quad\wedge \downdiff(\neg treeP(y))) \wedge \neg \exists y. (child(x,y) \wedge \neg \bothdiff(treeP(y)))\\
  &= p(x) \wedge \exists y. (child(x, y)\\
  & \quad\wedge \updiff(treeP(y))) \wedge \neg \exists y. (child(x,y) \wedge \neg \bothdiff(treeP(y)))\\
\end{align*}
\begin{align*} 
  &\downdiff(treeP(x))\\
  &= \downdiff(p(x) \wedge \neg \exists y. (child(x, y) \wedge \neg treeP(y)))\\
...
facts.\footnote{The ``support'' data structures introduced by
  \citep{gupta1993maintaining} are an attempt to avoid this issue by
...
\citet{demoor2013aggregates}, but for example this allows us to write
\begin{align*}
...
Beki\'c's Theorem \citep[][section 10.1]{winskel1993formal}, but it has a straightforward direct proof.
...
This is known \citep[see][and successors]{gupta1993maintaining},
...
\citep{avgustinov2016ql}, which produces large Datalog programs \textemdash{}
...
The seminal paper in this area is \citet{cai2014changes}. We deviate from
...
\emph{pointwise differences} \citep[see][section 2.2]{cai2014changes}. As
...
with ILC \citep[see][section 3]{cai2014changes}. The derivatives we give in \cref{sec:datalogDifferentiability} are more or
less a ``change semantics'' for Datalog \citep[see][section
3.5]{cai2014changes}. 
...
over the years (\citet{kilp2000monoids} is a good
...
homeomorphism in $\cat{SAct}$ \citep[see][]{kilp2000monoids} we can see that the structure is
...
\citet{arntz2017fixpoints} gives a derivative operator for fixpoints based on
the framework in \citet{cai2014changes}. However, since we have different
...
appears in \citet{bancilhon1986naive}. The idea of using an approximate derivative
and the requisite soundness condition appears as a throwaway comment in
\citet[][section 3.2.2]{bancilhon1986amateur}, and it would appear that nobody has since
...
expressions. In particular \citet{griffin1997improved} following
\citet{qian1991incremental} reveal the essential insight that it is necessary to
...
Several approaches
\citep{gupta1993maintaining,harrison1992maintenance}, most notably DReD
...
Other approaches \citep{dong2000incremental,urpi1992method} consider only
...
Datafun (\citet{arntz2016datafun}) is a functional programming language that embeds
...
In a different direction, \citet{cathcartburn2018hochc} have proposed
...
is the \emph{differential $\lambda$-calculus} \citep{ehrhard2003differential,ehrhard2017introduction}.
...
studied in depth
\citep{bucciarelli2010categorical,blute2010convenient,cockett2016categorical,kerjean2016mackey},
...
Automatic differentiation \citep{griewank2008evaluating} is a technique that allows
for efficiently computing the derivative of arbitrary programs, with
applications in probabilistic modeling \citep{kucukelbir2017automatic}
and machine learning \citep{baydin2014automatic} among other areas. In recent times, this technique has been successfully
applied to higher-order languages \citep{siskind2008nesting,baydin2016diffsharp}.
While some approaches have been suggested \citep{manzyuk2012simply,kelly2016evolving}, a general
...
be based \citep{kelly2016evolving}. We believe our change actions are better suited
