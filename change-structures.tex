...
Moor and Baars \cite{demoor2013aggregates}, but for example this allows us to write
\begin{align*}
...
So far we have defined change actions for the kinds of things that typically
make up \emph{data}, but we would also like to have change actions on
\emph{functions}. This would allow us to define derivatives for higher-order
languages (where functions are first-class); and for semantic operators
like fixpoint operators $\fixpoint : (A \rightarrow A) \rightarrow A$, which
also operate on functions.

Function spaces, however, differ from products and disjoint unions in that there
is no obvious ``best'' change action on $A \rightarrow B$.
Therefore instead of trying to define a single choice of change action, we will instead
pick out subsets of function spaces which have ``well-behaved'' change actions.

\begin{definition}[Functional Change Action]
...
There are two reasons why functional change actions are usually associated with
a \emph{subset} of  $U \subseteq A \rightarrow B$.
Firstly, it allows us to restrict ourselves to spaces of monotone or continuous
...
\begin{proposition}
  ...
  \begin{displaymath}
    \derive{f}(x, \change{x}) = \derive{\ev}((f, x), (\mzero, \change{x}))
  \end{displaymath}
\end{proposition}
...
\begin{proposition}
  ...
  we have:
  \begin{displaymath}
    (f \cplus \delta f)(x) = f(x) \cplus \ev'((f, x), (\delta f, \mzero))
  \end{displaymath}
\end{proposition}
...
\begin{proposition}
  ...
  a monoid with addition $+_\Rightarrow$ and neutral element $\mzero_\Rightarrow$ defined by:
  \begin{align*}
  ...
  \item $W$ is the monoid generated by $V$ and $+_\Rightarrow$, where $V$ is defined as:
    \begin{gather*}
      ...
      V \defeq \{~
          \lambda f. \lambda x. \ev'((f, x), (\delta f, \mzero)) ~|~ \delta f \in \changes U
          ~\}\\
...
Even if we restrict ourselves to the differentiable functions between $\cstr{A}$
and $\cstr{B}$ it is hard to find a concrete functional change action for this set.
Fortunately, in many important cases there is a simple change action
on the set of differentiable functions.

\begin{definition}[Pointwise functional change action]
  ...
  the monoid structure $(A \rightarrow \changes{B}, \splus_\rightarrow, \mzero_\rightarrow)$ and the action 
  ...
  \begin{align*}
    ...
    \mzero_\rightarrow (x) &\defeq \mzero_B
  \end{align*}
\end{definition}
...
The above definition is not always well-typed, since given $f : \cstr{A} \rightarrow \cstr{B}$ and
$\delta f : A \rightarrow \changes{B}$ there is no guarantee that 
$f \cplus_\rightarrow \delta f$ is differentiable. We present two sufficient
criteria that guarantee this.

\begin{theorem}
  \label{theorem:functionalCAct}
  ...
  $(\cstr{A} \rightarrow \cstr{B}, A \rightarrow \changes{B},
  \cplus_\rightarrow)$ is well defined.\footnote{Either
    of these conditions is enough to guarantee that the pointwise functional change action
...
\end{theorem}
As a direct consequence of this, it follows that whenever $L$ is a Boolean
algebra (and hence has a complete change action), the
...
\begin{proposition}[Derivatives of the evaluation map]
...
map exists \textemdash{} a pointwise change action actually gives us a
definition of it. In practice, this means
...
important class of posets. They allow us to take
\emph{fixpoints} of (Scott-)continuous maps, which is 
important for interpreting recursion in program semantics.
...
the monoid operation and action are (Scott-)continuous.
...
Unlike posets, the change order $\changeOrder$ does \emph{not}, in general,
...
\begin{rep-proposition}{booleanAlgebraContinuous}[Boolean algebra continuity]
...
of the derivatives of the $f_i$. A consequence of \cref{prop:distributivityLimit} 
...
\begin{corollary}
  ...
  $I$-indexed directed sets of functions in $A \rightarrow B$ and $A \times \changes{A} \rightarrow \changes{B}$ respectively.
...
We will later apply this technique to create a derivative for the semantics of
full recursive Datalog (\cref{sec:datalogIncr}).
...
$\cstr{A}$, and any reference to the change action $\cstr{\NN}$ will refer to
the monoidal change action on the naturals defined in \cref{sec:examples}.
...
\begin{rep-proposition}{iterDerivativesN}[Derivative of the iteration map with respect to $n$]
  ...
  \begin{align*}
    ...
    &\partial_2{\iter}(f, \mzero, m) \defeq \iter(f, m) \cminus \iter(f, 0)\\
...
semi-naive evaluation works: you compute the delta relation and the accumulator
simultaneously, adding the delta into the accumulator at each stage until it
becomes the final output.

\begin{rep-theorem}{fixpointIter}[Incremental computation of least fixpoints]
\label{theorem:fixpointIter}
  ...
  \bot)))$.\footnote{Note
    that we have \emph{not} taken the fixpoint of $\nextiter_f$, since it is
    not continuous.}
\end{rep-theorem}
\ifproofs
\begin{proof}
...
This can be seen as applying a change to the \emph{function} whose fixpoint we are
taking. We go from computing the fixpoint of $F(E, \_)$ to computing the
fixpoint of $F(E \cplus \change{E}, \_)$. If we have a pointwise functional
change action then we can express this change as a function giving the change at
each point, that is:
\begin{displaymath}
...
for the fixpoint operator $\fixpoint$: we want to know how the resulting
fixpoint changes given a change to its input function.

\begin{definition}[Derivatives of fixpoints]
...
consists of many steps of applying the derivative of the evaluation
...
\begin{rep-proposition}{iterDerivativesF}[Derivative of the iteration map with respect to $f$]
  ...
  \begin{align*}
    ...
    &\partial_1{\iter} (f, \change{f}, \mzero) \defeq \bot_{\changes{A}}\\
...
mutual recursion.\footnote{In
  fact, the recursion here is not \emph{mutual}: the first component does not
...
This indeed provides us with a function whose limit we can take. If we do so we
will discover that it is exactly $\derive{\lfp}$ (defined as in
\cref{def:fixpointDerivatives}), showing that $\derive{\lfp}$ is a true derivative.

\begin{rep-theorem}{leastFixpointDerivatives}[Derivatives of least fixpoint operators]
  \label{theorem:leastFixpointDerivatives}
  Let
  \begin{itemize}
...
to apply \cref{theorem:fixpointIter} (and hence we can incrementally
compute fixpoints that happen to exist even though the generating function is
not monotonic), but it is required to apply \cref{theorem:leastFixpointDerivatives}.
...
results from \cref{sec:fixpoints}, we have now created \emph{modular} proofs for
the two main results, which allows us to preserve them in the face of changes to
the underlying language.

\begin{corollary}
\label{theorem:diffEval}
  Datalog program semantics can be evaluated incrementally.
\end{corollary}
\ifproofs
\begin{proof}
  Corollary of \cref{theorem:fixpointIter} and \cref{corollary:consqDiff}.
\end{proof}
\fi

\begin{corollary}
\label{theorem:diffUpdate}
  Datalog program semantics can be incrementally maintained with changes to
  relations.
\end{corollary}
\ifproofs
\begin{proof}
  Corollary of \cref{theorem:leastFixpointDerivatives} and \cref{corollary:consqDiff}.
\end{proof}
\fi

Note that our approach makes no particular distinction between changes to the
\emph{extensional} relations (adding or removing facts), and changes to the
\emph{intensional} relations (changing the definition). The latter simply
amounts to a change to the denotation of that relation, which can be
incrementally propagated in exactly the same way as we would propagate a change
...
The seminal paper in this area is Cai et al. \cite{cai2014changes}. We deviate from
...
Cai et al. cite this as their reason for using a dependent
...
be forced to work with e.g. multisets admitting negative arities, as Cai et al. do.

Our function changes (when well behaved) correspond to what Cai et al. call
\emph{pointwise differences} (see \cite[section 2.2]{cai2014changes}). As
...
with their Incremental Lambda Calculus (see \cite[section 3]{cai2014changes}). The derivatives we give in \cref{sec:datalogDifferentiability} are more or
less a ``change semantics'' for Datalog (see \cite[section
3.5]{cai2014changes}). 
...
Arntzenius \cite{arntz2017fixpoints} gives a derivative operator for fixpoints based on
the framework in Cai et al.\cite{cai2014changes}. However, since we have different
...
do not require our changes to always be increasing.
...
Bancilhon and Ramakrishnan \cite[section 3.2.2]{bancilhon1986amateur}, and it would appear that nobody has since
...
Griffin, Libkin, and Trickey \cite{griffin1997improved} following
Qian and Wiederhold \cite{qian1991incremental} compute differences with both an ``upwards'' and a
``downwards'' component, and produce a set of
rules that look quite similar to those we derive in
\cref{theorem:concreteDatalog}. However, our presentation is significantly more
generic, handles recursive expressions, and works on set semantics
rather than bag semantics.\footnote{The same approach of finding derivatives
would work with bag semantics, although unfortunately the Boolean algebra
structure is missing.}

Several approaches
\cite{gupta1993maintaining,harrison1992maintenance} \textemdash{} most notably
DReD \textemdash{}
remove facts until one can start applying the
rules again to reach the new fixpoint. Given a good way of deciding what
facts to remove this can be quite efficient.
However, such techniques tend to be tightly coupled to the domain.
Although we know of no theoretical reason
...
Other approaches \cite{dong2000incremental,urpi1992method} consider only
restricted subsets of Datalog, or incur other substantial constraints.

\subsubsection{Embedding Datalog}
\label{sec:embeddingDatalog}

Datafun (Arntzenius and Krishnaswami \cite{arntz2016datafun}) is a functional programming language that embeds
...
In a different direction, Cathcart Burn, Ong, and Ramsay \cite{cathcartburn2018hochc} have proposed
...
is the \emph{differential $\lambda$-calculus} \cite{ehrhard2003differential,ehrhard2017introduction}.
...
studied in depth
\cite{bucciarelli2010categorical,blute2010convenient,cockett2016categorical,kerjean2016mackey},
...
Automatic differentiation \cite{griewank2008evaluating} is a technique that allows
for efficiently computing the derivative of arbitrary programs, with
applications in probabilistic modeling \cite{kucukelbir2017automatic}
and machine learning \cite{baydin2014automatic} among other areas. In recent times, this technique has been successfully
applied to higher-order languages \cite{siskind2008nesting,baydin2016diffsharp}.
While some approaches have been suggested \cite{manzyuk2012simply,kelly2016evolving}, a general
...
be based \cite{kelly2016evolving}. We believe our change actions are better suited
...
change actions. This has begun to be explored by the authors in \cite{alvarez2019changeactions}, where
change actions are generalized to arbitrary Cartesian base categories and a construction is provided to
obtain ``canonical'' Cartesian closed categories of change actions and differentiable maps.

We hope that these generalizations would allow us to extend the theory of change actions towards
other classes of models, such as synthetic differential geometry and domain theory.
Some early results in \cite{alvarez2019changeactions} also indicate a connection between 2-categories
and change actions which has yet to be fully mapped.

The compositional nature of these techniques suggest that an approach like that
used in \cite{elliott2018simple} could be used for an even more generic approach
to automatic differentiation.

In addition, there is plenty of scope for practical application of the
techniques given here to languages other than Datalog.
...
\subsection{Worked example of updating a recursive Datalog program}
\label{sec:workedExample}

The algorithm in \cref{theorem:leastFixpointDerivatives} is very abstract, and it is
...
As we have seen in \cref{theorem:leastFixpointDerivatives},
...
\begin{center}
  \begin{tabular} {p{4em} p{10em} p{10em}}
...
\section{Change order kernels and precision}
\label{sec:precision}
...
As we will see in the next section (\cref{sec:booleanAlgebrasPrecision}), minimal derivatives under
...
\subsection{Boolean algebras and precise derivatives}
\label{sec:booleanAlgebrasPrecision}

Boolean algebras have a nice definition of the kernel ordering, which gives us a
...
\begin{rep-proposition}{booleanAlgebraKernels}[Boolean algebra kernels]
...
\subsection{Precision of the concrete Datalog derivative}

In practice, while the derivative given in \cref{theorem:concreteDatalog} is not
precise, most of the cases are \emph{preciseness-preserving}, in that if the
subsidiary recursive cases are precise, then so is that case.
...
\repeatproposition{disjointUnions}
\begin{proof}
...
\repeatproposition{booleanAlgebraKernels}
\begin{proof}
...
\begin{proof}
  ...
  \begin{proposition}[Distributivity of $\sigma_\Gamma$]
...
\repeatproposition{booleanAlgebraContinuous}
\begin{proof}
...
Beki\'c's Theorem \cite[section 10.1]{winskel1993formal}, but it has a straightforward direct proof.

\begin{rep-proposition}{factoringFixpoints}[Factoring of fixpoints]
...
\repeatproposition{iterDerivativesN}
\begin{proof}
...
\repeatproposition{iterDerivativesF}
\begin{proof}
...
\repeattheorem{leastFixpointDerivatives}
\begin{proof}
