...
Consider the following archetypal Datalog program, which computes the transitive
...
this by repeating applying the rule, starting from empty. For example, supposing
that $e = \{ (1, 2), (3, 4), (5, 6) \}$:

\begin{center}
  \begin{tabular} {|c|c|c|}
    \hline
    Iteration & Deduced facts & Accumulated contents of $tc$ \\
    \hline
    1 & $\{ (1, 2)\}$ & $\{ (1, 2) \}$\\
    2 & $\{ (1,2), (3,4) \}$ & $\{ (1, 2), (3,4) \}$\\
...
But the delta rule translation works only for traditional Datalog. It is easy to
liberalise our syntax to support, say, disjunction, existential quantification, and parity-stratified
...
Then we can write programs like the following:
\begin{align*}
...
like to be able to use semi-naive evaluation for this rule too, but the delta
transformation does not work, and it is unclear how to extend it (and the
...
Semmle's implementation includes parity-stratified negation other non-standard
features, so a more generic approach to incremental evaluation is vitally necessary.
...
3.2.2]{bancilhon1986amateur}. If we can make this precise, then perhaps we can
work out how to take derivatives of more complex rules.

The idea of performing incremental computation using derivatives has been
studied recently by \textcite{cai2014changes}, who given an account using
\emph{change structures}. They use this to provide a framework for incrementally evaluating lambda calculus programs.

However, their work isn't directly applicable to Datalog: the tricky part
of Datalog's semantics is the \emph{fixpoint}, and we need some theory to tell us how to
...
derivative has a deep connection to the incremental evaluation of functions.
...
One obstacle to this approach is giving a denotation to negations. We adopt the usual
...
\todompj{mention the integration/integral curves stuff?}
