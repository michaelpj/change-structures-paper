...
derivative.\footnote{In several places we will need to pick an arbitrary
  ...
  operator for our domain, which alleviates the problem.} Functions into ``thin'' change
...
this order sometimes correspond to ``precise'' derivatives, in the sense of derivatives that do not perform
excessive work.
...
\label{thm:concreteDatalog}
  ...
  Then $\updift(T)$ and $\downdiff(T)$ are disjoint, and for any schema $\Gamma$
...
\label{thm:fixpointPseudoDerivatives}
  Let $\cstr{A}$, $\cstr{U} \subseteq \cstr{A} \Rightarrow \cstr{A}$, $\fixpoint_{U}$, and 
...
exactly the derivatives described in \cref{fig:datalogDerivatives}. We used
Semmle's profiling infrastructure, which runs over 800 queries (compiled to
...
In this case the versions we used were:
\begin{itemize}
  \item A ``full'' version incremental evaluation strategy described in this paper
