...
\begin{defn}[Change actions]
  ...
  We may abbreviate $\cstruct{A}{\changes{A}}{\cplus}$ to $\cstr{A}_\cplus$ or $\cstr{A}$.
%  \lo{We have already defined $\cstr{A} \defeq \cstruct{A}{\changes{A}}{\cplus_A}$. Do we ever write $\cstr{A}_\cplus$?} 
When there is no risk of confusion, we will simply write $\cplus$ for $\cplus_A$.
\end{defn}

\subsubsection{Examples}
\label{sec:examples}
A typical change action is \lochanged{$\cstruct{A^\ast}{A^\ast}{\doubleplus}$ where $A^\ast$ is the set of finite words (or lists) of $A$}. 
\lo{The notation $[A]$ may be confusing: $A$ is a set, which is part of a ``mathematical'' definition, but $[A]$ is the PL notation of list type.}
...
\begin{itemize}
  \item $\cstr{A}_1 \defeq \cstruct{A}{M}{\lambda(a, \change{a}). a}$, where $M$ is any monoid,
    which we call the \emph{discrete} change action on any base set, since it induces no changes at all.
    \lo{I think \emph{empty} or \emph{null} (which are opposite of full) may be a better name than discrete.}
...
\begin{defn}[Derivatives]
  ...
  the set of differentiable functions \lochanged{from $A$ to $B$}.\footnote{Note that we do not require that $\derive{f}(a,
...
\begin{prop}[Completeness equivalences]
  ...
  \begin{itemize}
    ...
    \item \lochanged{Any function $f: {B} \rightarrow {A}$ is differentiable, where $\cstr{B}$ is an arbitrary change action.}
  \end{itemize}
\end{prop}
...
\label{thm:derivativeCharacterization}
  ...
  \lochanged{$f: {A} \rightarrow {B}$} be a function. If there exist $\subderiveM{f}$ and
...
\begin{defn}[Schema]
  A schema $\Gamma$ is a finite set of names. A named tuple over $\Gamma$ is an assignemt
...
\lochanged{``neXt''} value of a term by applying the upwards and downwards derivatives.
