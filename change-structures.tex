...
Our definition uses an auxilliary function, $\bothdiff$, which computes the ``neXt'' value of a term by applying the upwards and downwards derivatives.
As we expect
...
\begin{proof}
  ...
  therefore $\bot \kernel_{(r, s)} p$. This implies
  ...
  \begin{align*}
    p \wedge s &\leq b \twist (p, q)\\
               ...
               &\leq \neg s
  \end{align*}
  Hence $p \wedge s \leq \neg s$, therefore $p \wedge s = \bot$, which concludes the proof.
\end{proof}
