...
\begin{prop}[name=Products, restate=products]
  ...
  \changes{B}}{\cplus \times \cpluss}$ is a change action, and the projection
maps $\pi_1$,$\pi_2$
...
\begin{prop}[name=Coproducts, restate=coproducts]
  ...
  \changes{B}}{\cplusvee}$ is their categorical coproduct, and the injection
maps $i_1$, $i_2$ are differentiable with $\cplusvee$ defined as:
  \begin{align*}
...
\begin{thm}[Sandwich lemma]
  ...
  \changes{A}$. If $\supderive{f}$ and $\subderive{f}$ are
...
\begin{defn}[Pointwise change actions]
  ...
  $\cstruct{A \rightarrow B}{A \rightarrow \changes{B}}{\cplus_\rightarrow}$,
