...
\begin{figure}[t]
  \fbox{
    ...
    \vspace{6pt}
    \begin{align*}
      \bothdiff(R) \defeq R \twist (\updiff(R), \downdiff(R))
    \end{align*}
    \end{minipage}
  }
...
change actions. There is a simple category $\cat{CAct}$ of change actions and differentiable
functions between them, which turns out to be equivalent to $\cat{PreOrd}$ and
has interesting Cartesian closed subcategories.

Furthermore, $\cat{CAct}$ could be generalized in two additional ways: as a
2-category (corresponding to the 2-categorical interpretation of $\cat{PreOrd}$,
and as a category over arbitrary base categories (rather than just $\cat{Set}$).
...
\repeatproposition{factoringFixpoints}
\begin{proof}
