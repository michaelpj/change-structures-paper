...
\begin{prop}
  ...
  Therefore 
  $$(f \cplus \change{f})(a \cplus \change{a}) = f(a) \cplus \derive{f}(a, \change{a}) \splus \change{f}(a \cplus \change{a})$$
\end{prop}
...
\begin{proof}
  ...
  \item[ ]
    ...
    $
  \item[=]\{ definition of $\Delta(A \rightarrow A)$ \}\\
    $
    f(\thefixpoint \cplus \change{w}) \cplus \change{f}(\thefixpoint \cplus \change{w})
    $
  \item[=]\{ differentiability of $f$ \}\\
    $
    f(\thefixpoint)
    \cplus
    \left[ f'(\thefixpoint, \change{w}) + \change{f}(\thefixpoint \cplus \change{w}) \right]
    $
  \item[=]\{ rolling the fixpoint \}\\
    $
    \thefixpoint
    \cplus
    \left[ f'(\thefixpoint, \delta w) + \change{f}(\thefixpoint \cplus \change{w}) \right]
    $
  ...
  Hence $F \cplus \change{w}$ is a fixpoint of $f \cplus \change{f}$.
  ...
  \change{f})$ is the least fixpoint of $f \cplus \change{f}$. As we have seen,
  $D(f, \change{f})$ satisfies equation \ref{eqn:fixcondition} and, therefore,
  by the least fixpoint property, we have $\fixpoint'_A(f, \change{f}) \leq D(f, \change{f})$.
  Since $\cplus$ is monotonic on its second argument, we have:
  $$
    \fixpoint_A(f \cplus \change{f}) \leq 
    \fixpoint_A(f) \cplus \fixpoint'_A(f, \change{f}) \leq
    \fixpoint_A(f) \cplus D(f, \change{f}) = \fixpoint_A(f \cplus \change{f})
  $$
  Hence $\fixpoint_A(f \cplus \change{f}) = \fixpoint_A(f) \cplus \fixpoint'_A(f, \change{f})$
