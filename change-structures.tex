...
At this point we have reached a fixpoint, so the evaluation ends.
...
incremental maintenance of already computed fixpoint expressions, given a change to
...
computation and maintenance of recursive Datalog programs, showing how to handle 
language constructs such as negation, disjunction, and additional well-behaved
extensions (\cref{sec:nonRecursiveDatalog}, \cref{sec:recursiveDatalog}). This provides, to the best
of our knowledge, the world's first incremental
evaluation and maintenance mechanism for Datalog that can handle these features. Moreover, the structure of the
proof is modular, and can accommodate arbitrary additional
formula constructs (\cref{sec:extensions}). The technique for handling fixpoints
is also applicable to other programming languages.
...
A typical change action is $\cstruct{[A]}{[A]}{\doubleplus}$. Here we can change
a list (or stream) by concatenating another stream onto it. The changes
...
\begin{itemize}
  ...
  \item $\{A\}$, the type of sets, has change actions
    $\cstruct{\{A\}}{\{A\}}{\cup}$, $\cstruct{\{A\}}{\{A\}}{\cap}$.
...
function at a point, and a change to that point, they tell you how to change the
the function value to get the new value.
...
For our Datalog case study, we are going to want a derivative of the semantics of
Datalog. As we will discuss later (\cref{sec:nonRecursiveDatalog}, \cref{sec:recursiveDatalog}), we can see Datalog's
...
\todomario{
  I'm inclined to make this a sub-subsection of the work on preorders since it doesn't really add
  anything to it, and the theorems on dcpos really ought to be moved to the part where they're used.
}
...
A direct corollary of this property is the following result, reminiscent of the well-known result from
standard calculus that the limit of an absolutely convergent sequence of differentiable functions is
...
The change ordering corresponds to a natural notion of precision: if $\change{a}
...
The most precise deriviatives are therefore those that are minimal with respect
to $\changeOrder$.
...
In a boolean algebra, this corresponds to being the unique minimal derivative.
...
other properties, such as how easy the derivative is to compute.
...
looking at the non-recursive part of the semantics.

We will use Datalog as our example language, but the approach is generic.
...
While recursion is a major feature of Datalog, the majority of its semantics is
the non-recursive semantics of Datalog formulae.
...
$\denote{T}$. The derivative that we know we have for complete change actions
is quite bad:
\begin{displaymath}
  \derive{f}(a, \change{a}) = f(a \cplus \change{a}) \cminus f(a)
\end{displaymath}
While this is precise, if we compute this naiveley, it requires \emph{more}
work than evaluating $f(a \cplus \change{a})$ directly!
...
\todompj{Simplified version of the exponentials material}
...
We will do this later to full recursive Datalog (\cref{sec:datalogIncr}).
...
operator itself. The typical use case for this is where we have some fixpoint
\begin{displaymath}
...
\label{def:fixpointDerivatives}
  Let $\cstr{A}$ be a change action, $\fixpoint$ a fixpoint operator for
...
lattice \textemdash but this may still be significantly less expensive than the
...
semantics, as well as being able to compute the fixpoints themselves
incrementally. 

Again, we will demonstrate the technique with Datalog, but the approach is generic.
...
the question - is there a category here? And is it Cartesian closed?
...
\todompj{On reflection (hah) I think we can do better: I think complete change
  actions are a reflective subcategory, where the reflector is the
  ``completion'' of a change action i.e. pick any complete change action on the
  set. Or do we? We do if it's set based, since we can use $A \rightarrow A$ but
  perhaps there are also ``uncompletable'' change actions.}
...
The earliest interpretation of semi-naive evaluation as a derivative process
