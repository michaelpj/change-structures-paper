...
\todompj{Get rid of the overlines in favour of indices, will be clearer for most people.}
...
\begin{defn}[Formula semantics]
  ...
  \begin{align*}
    ...
    \denote{T}(\overline{R}) &= \left\{ \left(\overline{a} \right) \mid \Phi \models T\left[ \overline{R}, \overline{a} \right] \right\}
  \end{align*}
\end{defn}
...
\label{thm:concreteDatalog}
  ...
  that $\denote{\updiff(T)}(\overline{\updiff R}) \times
  \denote{\downdiff(T)}(\overline{\downdiff R})$ is a derivative for
  the semantics of a Datalog formula $T$.
...
The upwards difference in particular is still not especially easy to compute. If we naively compute it, the
third conjunct requires us to recompute the whole of the recursive part. However,
the second conjunct gives us a
guard: we only need to evaluate the third conjunct if there is \emph{some} change in the body of
our existential.
