...
\newcommand{\todomario}[1]{\todo[inline,color=yellow!40,author=Mario]{#1}}
...
\todokcg{This is a bit of an abrupt change. A less-expert reader (cough cough) might like to have
  this jump a bit cushioned with some motivation: why do we make this definition and what is it
  intuitively describing? What is the previous definition of a change set, and how does this
  differ?}

\todokcg{You should use the align* environment for equations or you get super janky spacing.}
...
Elements in the change set represent changes that can be made to elements in the base set, with the
monoid action being the operation that ``applies'' the change. \todokcg{I think the observation
  (that you can just take the free monoid) distracts a bit from the intuition here and would perhaps
  be better as a footnote} The requirement that the change set be a monoid is convenient but in fact
...
\todokcg{I have no idea what a transformation monoid is, so this is perhaps less illuminating for me
  than might be hoped.}
...
\todokcg{You haven't defined $\cstr{A}_\cplus, \cstr{B}_\cpluss, \changes{A}, \text{ or }\changes{B}$}
\end{defn}
...
\end{thm}
\todokcg{I think you certainly don't need this proof here. Stick it in an appendix and say ``by
  arithmetic'' here.}
\ifproofs
\begin{proof}
  By equivalence:
...
\todokcg{I don't think you need quite this many examples. Pick one simple one, one medium one and
  one wat one.}
...
\begin{itemize}
  ...
  \item $\cstr{A}_\leq \defeq \cstruct{A}{\leq}{ev_\leq}$, where $\leq$ is some
    ...
    \todokcg{Why is this conditional \emph{application}?}
...
\todokcg{I would get rid of all the defn environments and just write in plaintext. It's hard to
...
\begin{proof}
  ...
  \begin{itemize}
    \item[ ]$\iter_f((n+1) + m)$
    \item[=]\{ definition of $\iter_f$ \}\\
      $f(\iter_f(n + m))$
    \item[=]\{ by induction \}\\
      $f(\iter_f(n) \cplus \derive{\iter_f}(n, m))$
    \item[=]\{ $f$ is differentiable, definition of $\iter_f$ \}\\
      $\iter_f(n+1) \cplus \derive{f}(\iter_f(n), \derive{\iter_f}(n, m))$
  \end{itemize}
\end{proof}
\fi
...
\begin{proof}
  ...
  \begin{itemize}
    \item[ ]
      $\lfp(\iter_f)$
    \item[=]$\sqcup_{n \in \NN} \iter_f(n)$
    \item[=]$\sqcup_{n \in \NN} \pi_1 (\nextiter_f^n(\bot))$
    \item[=]\{ $\pi_1$ is continuous \}\\
      $\pi_1 (\sqcup_{n \in \NN} \nextiter_f^n(\bot))$
