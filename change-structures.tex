...
The standard improvement to this evaluation strategy is what's commonly known as ``semi-naive'' 
...
computation of Datalog (and other) programs, and show how it can be used to account for extensions
to 
...
having nice categorical behaviour (\cref{sec:category}) and existing for more structures (\cref{sec:moreStructures}).
...
This provides, to the best of our knowledge, the world's first incremental
...
change structures on their Cartesian product $A \times B$ or disjoint union $A + B$. While there are
many candidates, there is a clear ``natural'' choice for either.
...
\begin{prop}[name=Products, restate=products]
  ...
  \changes{B}}{\cplus \times \cpluss}$ is a change structure, and the projection maps $\pi_1, \pi_2$
  are differentiable with respect to it.

  \todomario{
    This result isn't super interesting, but it's reminiscent from calculus so I like it. Plus,
    there's no dual to this for coproducts, so I feel like it's worth pointing it out, but if we need
    space it goes away.
  }
  Furthermore, a function 
  $f : A \times B \into C$ is differentiable as a map from $\cstr{A} \times \cstr{B}$ into $\cstr{C}$ if
  and only if, for every $a \in A$ and $b \in B$, the partially applied functions 
  \begin{align*}
    f(a, \cdot) : B \into C\\
    f(\cdot, b) : A \into C
  \end{align*}
...
In fact, any change action whose base set is a poset induces a particularly convenient preorder
...
\begin{prop}
  Let $\cstr{A}$ be a change action on a set $A$ equipped with a preorder $\leq$ such that
  $\cplus$ is monotone in its first argument. Then $\cstr{A}$ is an ordered change action when
  $\changes{A}$ is equipped with the preorder $\changeOrder$.
\end{prop}

In what follows, we will abuse the notation and extend the preorder $\changeOrder$ on some change
set $\cstr{A}$ to functions from some $B$ into $\cstr{A}$. This pointwise preorder is well-behaved
...
\todompj{Something about kernel orderings?}
...
\todomario{
  I'm inclined to make this chunk a sub-subsection of the work on preorders since it doesn't 
  really add anything to it, and the theorems on dcpos really ought to be moved to the part where
  they're used.
}

\subsubsection{Directed-complete partial orders}

An important class of posets orders, of especial importance in semantics, is that of 
directed-complete partial orders (dcpos) 
equipped with a least element, and the associated notion of \emph{(Scott) continuous} maps. 
\todomario{Maybe write the basic definitions of dcpo and continuous maps?}
As in the previous section, we can define change actions on dcpos, rather than sets, as change
actions whose sets $A, \Delta A$ are endowed with a dcpo structure, and where the maps $\oplus, +$ are
continuous.
...
\todomario{Not super convinced as to whether number remarks}
\begin{rem}
  Unlike in the previous section, the change order $\changeOrder$ does \emph{not}, in general,
  induce a dcpo structure on $\changes{A}$. As a counterexample, consider 
  the change structure $(\NN^\star, \NN, +, +, 0)$, where $\NN^\star$ denotes the dcpo of natural numbers
...
\end{proof}
\fi
\todomario{It bothers me slightly that these theorems appear here and are then never used until much
later, maybe move to the place of use}
...
Datalog programs. Conveniently, these structures can be endowed with a complete, continuous change
action.
...
\begin{prop}[name=Boolean algebra change actions, restate=lsuperpose]
 Let $L$ be a complete Boolean algebra. Define
  ...
  The monoid operation on $L \times L$ is given by:
  \begin{displaymath}
    (p, q) \splus (r, s) \defeq ((p \wedge \neg q) \vee r, (q \wedge \neg r) \vee s)
  \end{displaymath}
  with identity element $(L, \bot)$.
...
``downwards'' changes, where the monoid action simply applies one after the other.\footnote{
...
\label{cor:booleanCharacterization}
  Let $L$ be a Boolean algebra considered with the corresponding change action $\cstr{L}_\superpose$, $A$ beA
  an arbitrary change action, and $f: A \rightarrow
...
We set our sights now on applying the theory defined so far to the specific setting of the semantics
of Datalog. Giving a semantics for it that admits derivatives will then lead us to a strategy for
performing incremental computation of Datalog programs. 
We will first restrict ourselves to the non-recursive fragment of the language, and we will tackle
fixpoints in a later section (\cref{sec:recursiveDatalog}).

In principle, any first-order language could have been used as a motivating example. 
We have decided on Datalog, a well-known simple logic programming language
\autocite[See][part D]{abiteboul1995foundations}. This was partly for practical reasons, but it is
also the canonical example of the need for incremental computation since, as we have seen
(\cref{sec:intro}), the naive computation of its semantics can be very expensive, and significant
performance gains can be obtained by incrementalization.

\subsection{Semantics of Datalog formulae}

Datalog is usually given a logical semantics wherein facts are interpreted as first-order
logic formulae and the semantics of a program is precisely the set of models of its constituent
formulae. We will instead give a simpler denotational semantics that treats a Datalog
...
derivative for $\denote{T}$. 
Consider, for example, the minimal derivative $\derive{\denote{T}}_{\cminus_\bot}$ as defined in 
\cref{prop:minimalMaximalDerivatives}. Then:
\begin{align*}
  \derive{\denote{T}}_\cminus(a, (p, q)) &= \denote{T}(a \twist (p, q)) \cminus_\bot \denote{T}(a)\\
                                         &= (\denote{T}((a \vee p) \wedge \neg q) 
                                           \wedge \neg \denote{T}(a),
                                           \denote{T}(a) 
                                           \wedge \neg \denote{T}((a \vee p) \wedge \neg q))
\end{align*}
While this derivative is precise (in the sense of \cref{def:precision}),
it requires \emph{more} work to compute than computing $f(a \cplus \change{a})$ directly!

However, \cref{cor:booleanCharacterization} entails that \emph{any} function bounded by
$\derive{\denote{T}}_{\cminus_\bot}$ and $\derive{\denote{T}}_{\cminus_\top}$ is a valid derivative,
and we can therefore search within that range to find one that provides the desired
trade-off between ease of computation and precision.

\todomario{Rewrite this, because we don't quite have least and greatest derivatives}
...
set for $\widehat{\Rel}_\superpose$ is $\left( \Rel, \Rel \right)$, it
...
We define here a derivative operator as a pair of mutually recursive functions,
...
\todompj{We're talking about this as a semantic derivative, but with the way
  we've defined it it's also a derivative for formulae under
  denotational equivalence. Worth mentioning, perhaps.}

\todompj{This still isn't quite totally clear.}
...
\label{thm:concreteDatalog}
  ...
  defined by structural induction as in figure \ref{fig:datalogDerivatives}
  \todomario{cref references theorem 26 instead of figure 1 for some reason}

  Then given a Datalog formula $T$ and interpretations for its free relation variables
  $\denote{R_1} \ldots \denote{R_n}$, we have:
  \todomario{I have no idea how to make this fit the space.}
  \begin{gather*}
      \denote{T}(\denote{R_1} \twist (\denote{\updiff{R_1}}, \denote{\downdiff{R_1}}), \ldots,
                 \denote{R_n} \twist (\denote{\updiff{R_n}}, \denote{\downdiff{R_n}})) \\=\\
    \denote{T}(\denote{R_1},\ldots,\denote{R_n}) \twist
    (\denote{\updiff{T}}(\denote{\updiff{R_1}}, \ldots, \denote{\updiff{R_n}}, 
                        \denote{\downdiff{R_1}},\ldots, \denote{\downdiff{R_n}}), 
    \denote{\downdiff{T}}(\denote{\updiff{R_1}}, \ldots, \denote{\updiff{R_n}}, 
                        \denote{\downdiff{R_1}},\ldots, \denote{\downdiff{R_n}}))
  \end{gather*}
...
Conveniently, our definition of function changes gives us a way to compute explicit derivatives for
the evaluation map:
...
This indeed provides us with a function that we can take the limit of, showing
...
We knew how to do this already (semi-naive evaluation), but our proof is more
