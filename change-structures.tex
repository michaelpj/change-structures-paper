...
(especially those in \cref{sec:fixpoints}) are included in an appendix (\cref{sec:proofs}), along
with some extended worked examples (\cref{sec:workedExamples}), and additional
material on the precision of derivatives (\cref{sec:precision}).
...
then we find ourselves with a monoid action.\footnote{Using
  monoid actions for changes gives us a reason to think that
...
denoted as $\updiff R$ and $\downdiff R$ respectively.\footnote{While
  the definitions usually exhibit the dualities we would expect
...
\subsection{Worked example of computing the derivative of a Datalog predicate}
...
\begin{rep-proposition}{iterDerivativesN}[Derivative of the iteration map with respect to $n$]
  ...
  \begin{align*}
    ...
    &\partial_2{\iter}(f, \mzero, m) \defeq \iter(f, m) \cminus \bot\\
