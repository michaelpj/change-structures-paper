...
\ifproofs
  ...
  is complete. Thus, trivially, any function $f : A \times C \rightarrow B$ is differentiable
  if and only if its curried version $\Lambda(f) : A \rightarrow B^C$ is.
\fi
...
\begin{prop}
  ...
  The pointwise exponential object $\exponential{\cstr{A}}{\cstr{B}}$ of change actions $\cstr{A}$ and $\cstr{B}$ is
  thin if $\cstr{B}$ is. Furthermore, whenever $\cstr{B}$ is complete, then so is $\exponential{\cstr{A}}{\cstr{B}}$.
\end{prop}
