...
will consider change actions over posets, which give us the important ability to
...
set $\cstr{A}$ to functions from some $B$ into $\cstr{A}$. This pointwise
preorder interacts nicely with derivatives, in that it gives us the following lemma:
...
\label{thm:concreteDatalog}
  ...
  Then $\derive{\denote{T}} \defeq (\denote{\updiff(T)}, \denote{\downdiff(T)})$
...
\subsection{Change actions over ordered structures}
