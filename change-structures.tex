...
For example, in $\cstruct{\mathcal{P}(A)}{\mathcal{P}(A)}{\cup}$, $\{1\} \changeOrder \{1,2\}$. Even
...
\todompj{Is this useful? Surely you can just do $f \cplus \change{f}$ normally?}
...
\todompj{What is $\upsilon$? Also missing $0_\Rightarrow$, generally seems incomplete.}
...
\todompj{Wasn't $\cstr{A} \rightarrow \cstr{B}$ going to be differentiable
  functions? We've drifted from that now, we should pick one thing.}
...
\todompj{backreference for $\cstr{\NN}$}
...
\todompj{I think we can simplify this a bit, in particular $f$ being continuous
  and differentiable is now implied, I think.}
