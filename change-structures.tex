...
on the base set.\footnote{The requirement that the change set be a monoid is convenient but in
  ...
  free monoid over the action set to obtain a monoid action.}\footnote{
Requiring the change set to be monoid action gives us a reason to think that
...
\todompj{These footnotes read weirdly together}
...
\todompj{I think we need to say something about minimality and why we're not
  worrying about it. Perhaps here?}
...
\todompj{Move after the more concrete bits? I think the only bit from here we
  need is the bits about the evaluation map for the fixpoint section, and we
  implicitly use products in a few cases.}
...
actions over the wide variety of data types that we actually use in programming.
...
for our proofs in \cref{sec:fixpoints}: in particular, the derivative of the exponential \emph{evaluation map}
$\ev$ has a deep connection to the incremental evaluation of functions (\cref{prop:incrementalization}).
...
\todompj{Maybe we should just define the subcategories? I think they are all
  straightforwardly CCCs if everything is pointwise, not sure in general.}
...
This means that we can often statically determine whether a formula will have a
precise derivative or not, which can be useful in practice.
...
\todompj{Worked example of updating a fixpoint? e.g. a TC. Would be painful, but
  probably helpful. Annoyingly, we haven't given the Datalog semantics yet.
  Perhaps we should do that after boolean algebras, then this section, then put
  the pieces together.}
...
recursive definitions using fixpoints. \Cref{sec:fixpoints} now gives us a way
to compute those fixpoints incrementally, as well as giving us a derivative for
our overall program semantics.
...
While we can't give a precise derivative, we can still provide a guard that prevents us from having to re-evaluate in all cases. 
