...
action, such as the full change action $\cstr{A}_\Rightarrow$,
...
re-evaluating the whole subformula, as the guard allows us to skip the second conjunct when $U$
has not changed.
...
\label{thm:fixpointPseudoDerivatives}
  Let $\cstr{A}$, $\cstr{U} \subseteq \cstr{A} \Rightarrow \cstr{A}$, $\fixpoint_{U}$, and let
...
Let $\cstr{A}$ a change action. 
We define ${\reachOrder} \subseteq A \times A$ by: $a \reachOrder b$ just if there is a $\change{a} \in \changes{A}$ such that $a \cplus_A
  \change{a} = b$.
\end{defn}
...
Conversely, any preorder $(A, \leq)$ induces a change action
...
The mapping $(A, \leq) \mapsto \cstr{A}_\leq$ gives rise to a (full and faithful) functor
...
\begin{prop}[name=Convenient change actions, restate=convenientChangeActionProps]
  ...
  In particular, every change action $\cstruct{B}{\changes{B}}{\cplus}$ where $\cplus$ is a group action is convenient.
\end{prop}
\ifproofs
...
well-established. One might ask whether we actually gain anything by expanding the set of predicates
...
timeouts. Further investigation reveals that these regress very badly indeed. For many of
...
It is possible to define a category $\cat{CAct}$ of change actions and differentiable
functions between them, as the identity function is trivially differentiable and the chain
rule guarantees that composition preserves differentiability. The product and sum of change
actions as described in section \cref{sec:prodsum} correspond precisely to products and
coproducts in this category. Furthermore, the result in \cref{thm:functionalCAct} can be
...
This has the immediate consequence that $\cat{CAct}$ is Cartesian closed.
...
the category that has elements of $A$ as objects and an arrow $f_{\change a} : a \ra b$ for
every change $\change a \in \changes A$ such that $a \oplus \change a = b$. If one requires
that derivatives verify the additional conditions in \cref{def:derivative}, then functions that
admit derivatives correspond exactly to functors.

This connection naturally leads to consider dependently-typed generalizations of
change actions where different elements $a, b \in A$ have different change sets
$\changes_aA, \changes_aB$, much like in \citeauthor{cai2014changes}'s original
presentation. The resulting theory, while perhaps less suitable for
implementation in non-dependently typed languages, is nonetheless valuable and
remains to be developed.

\subsection{Change actions on arbitrary categories}

A more abstract approach is also possible: throughout this work we have
restricted ourselves to change actions defined over the category of sets, but
most of the definitions and results presented here could be easily generalized
to any category with products. The resulting theory is the subject of ongoing
work by the authors.
