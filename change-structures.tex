...
\todomario{Maybe move this to a footnote}
...
fixpoint operators, which are %best thought of as 
...
of a ``best'' change action structure on \lochanged{(the set-theoretic function space)} $A \rightarrow B$, given change actions $\cstr{A}$ and $\cstr{B}$.
...
\begin{prop}[Differentiability and functional change actions]
  ...
  $f \in U$ is differentiable, with a derivative $f'$ given by:
  \begin{displaymath}
...
\todomario{
  There's a classification theorem here, but it's a bit tricky and I'm not sure it's worth including.
...
\begin{prop}[Functional change sets]
  ...
  \begin{align*}
    ...
    &\defeq \upsilon f (h, x) \oplus \upsilon g((\lambda x . \upsilon f (h, x)) , x)
  \end{align*}
...
$\cstr{A} \difffunc \cstr{B}$ to distinguish it from the set-theoretic function space
...
\begin{thm}[Pointwise functional change actions]
  ...
  \lochanged{(Moreover, it is the exponentiation object (up to unique isomorphism) from $\cstr{A}$ to $\cstr{B}$ in the category of change actions and differentiable functions (Section~\ref{sec:exponentials}).)}
  \footnote{
...
These pointwise change actions are, furthermore, functional in the sense of \cref{def:functionalChanges}. What's more, the derivative of the evaluation map is quite easy to 
compute.
...
\todomario{Not really necessary anymore since we can talk about partial derivatives freely}
...
\begin{thm}[name=Derivatives of least fixpoint operators, restate=leastFixpointDerivatives]
  ...
  \begin{itemize}
    ...
    \item $U$ be the set of continuous functions $f : A \rightarrow A$, 
      endowed with a functional change action $\cstr{U} \subseteq \cstr{A} \rightarrow \cstr{A}$
...
\todomario{Not super happy about calling these ``pointwise'' - maybe convenient? (by analogy to convenient vector spaces)}
...
\todomario{
  Beware this part, we should give the reflector explicitly and prove it preserves products.
  It is in fact much easier to establish this fact directly by showing $A \Rightarrow B$ is
  complete whenever $B$ is.
}
Since the complete change actions are a reflective subcategory of
...
\todomario{This is new, and it's kind of cool. It's also trivial to prove}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \sem{T}(\semR) \twist (\upsem{T}, \downsem{T}) \cap \sem{U}(\semR) \twist (\upsem{U}, \downsem{U})\\
    &= (\sem{T}(\semR) \cup \upsem{T}) \cap \neg \downsem{T} \cap (\sem{U}(\semR) \cup \upsem{U}) \cap \neg \downsem{U}\\
    ...
    &= \sem{T}(\semR) \cap \sem{U}(\semR) \cap \neg (\downsem{T} \cup \downsem{U})\\
      &\cup ((\upsem{T} \cap \bothsem{U}(\semR, \diffR))
            \cup (\upsem{U} \cap \bothsem{T}(\semR, \diffR)))\\
    &= \sem{T}(\semR) \cap \sem{U}(\semR) 
      \cup ((\upsem{T} \cap \bothsem{U})
            \cup (\upsem{U} \cap \bothsem{T}))
      \cap \neg (\downsem{T} \cup \downsem{U})\\
    &= \sem{T \wedge U}(\semR)
      \twist (\upsem{T \wedge U}, \downsem{T} \cup \downsem{U})\\
    &= \sem{T \wedge U}(\semR)
      \twist (\upsem{T \wedge U}, (\downsem{T} \cap \sem{U}(\semR)) \cup (\downsem{U} \cap \sem{T}(\semR)))\\
    &= \sem{T \wedge U}(\semR)
      \twist (\upsem{T \wedge U}, \downsem{T \wedge U})
  \end{align*}
