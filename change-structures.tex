...
\begin{prop}[restate=pointwiseChangeActions]
...
\begin{prop}[restate=pointwiseComplete]
...
is the \emph{differential $\lambda$-calculus}\citeauthor{ehrhard2003differential}\citeauthor{ehrhard2017introduction}.
...
While both systems have clear similarities, there are big differences between
them, the most important one being the properties of the derivatives themselves:
in the differential $\lambda$-calculus, derivatives are guaranteed to be linear
in their second argument, whereas in our approach, derivatives are unrestricted
in this sense
...
studied in depth\citeauthor{bucciarelli2010categorical}\citeauthor{blute2010convenient}\citeauthor{cockett2016categorical}
\citeauthor{kerjean2016mackey}, and the relationship between these and change actions
is the subject of ongoing work.

\subsection{Higher-order automatic differentiation}

Automatic differentiation\citeauthor{griewank2008evaluating} is a technique that allows
for efficiently computing
the derivative of arbitrary programs, with applications in probabilistic modeling\citeauthor{kucukelbir2017automatic}
and machine learning\citeauthor{baydin2014automatic} among other areas. In recent times, this technique has been successfully
applied to higher-order languages\citeauthor{siskind2008nesting}\citeauthor{baydin2016diffsharp}.
While some approaches have been suggested\citeauthor{manzyuk2012simply}\citeauthor{kelly2016evolving}, a general
theoretical framework for this technique is still
...
be based\cite{kelly2016evolving}. We believe our notion of change actions is intrinsically better suited
to this purpose than change structures, since one can easily give them a
...
ways. A dependently-typed version, as gestured towards in
\cref{sec:relatedChangeStructures}, might smooth over many of
the technical difficulties. A generic categorical description of change actions
would also be easy to provide, which may lead to models of
the incremental $\lambda$-calculus on different base categories. 

Of those, the theory of change actions on the category domains is of particular interest. Since
domains are commonly used to model recursion in programming language semantics, this could
...
\label{prf:pointwiseChangeActions}
  ...
  $\change{f} : A \rightarrow \changes{B}$ a pointwise change. Then:
  \begin{align*}
    (f \cplus_\rightarrow \change{f})(a \cplus \change{a}) &= f(a \cplus \change{a}) \cplus \change{f}(a \cplus \change{a})\\
  ...
  \begin{align*}
    \ev((f, \change{f}), (a, \change{a})) &= (f \cplus_\rightarrow \change{f})(a \cplus \change{a}) \\
  ...
  Now, on one hand, suppose that $f : C \times A \rightarrow B$ is a differentiable function,
  and $\Lambda(f) : C \rightarrow \cstr{B}^{\cstr{A}}$ its corresponding curried version:
  \begin{align*}
    \Lambda(f)(c \cplus \change{c}) &= \lambda a . f(c \cplus \change{c}, a)\\
    &= \lambda a . f(c, a) \cplus f'((c, \change{c}), (a, 0))\\
    &= (\lambda a . f(c, a)) \cplus_\rightarrow (\lambda a . f'((c, \change{c}), (a, 0)))
  \end{align*}
  Then the funciton $\lambda a . f'((c, \change{c}), (a, 0))$ is a derivative for $\Lambda(f)$,
  thus $\Lambda(f)$ is differentiable. Conversely, suppose that $\Lambda(f)$ is differentiable.
  Then we note that, by hypothesis, $\Lambda(f)(a)$ is differentiable for all $f$, with derivative
  $f'_a$, and hence:
  \begin{align*}
    f(a \cplus \change{a}, c \cplus \change{c}) &= \Lambda(f)(a \cplus \change{a})(c \cplus \change{c})\\
    &= (\Lambda(f)(a) \cplus_\rightarrow \derive{\Lambda(f)}(a, \change{a}))(c \cplus \change{c})\\
    &= \Lambda(f)(a)(c \cplus \change{c}) \cplus \derive{\Lambda(f)}(a, \change{a})(c\cplus\change{c})\\
    &= \Lambda(f)(a)(c) \cplus (f'_a(c, \change{c}) \splus \derive{\Lambda(f)}(a, \change{a})(c\cplus\change{c}))\\
    &= f(a, c) \cplus (f'_a(c, \change{c}) \splus \derive{\Lambda(f)}(a, \change{a})(c\cplus\change{c}))
  \end{align*}
...
\label{prf:pointwiseComplete}
  ...
  We prove that the pointwise change action is the exponential object $\cstr{B}^{\cstr{A}}$.
  ...
  Now consider two elements $\Lambda(f), \Lambda(g)$ of $\cstr{B}^{\cstr{A}}$. Since
  $\cstr{B}$ is complete, it is endowed with a minus operator $\cminus$, so we define
  $\cminus_\rightarrow : \cstr{B}^{\cstr{A}} \times \cstr{B}^{\cstr{A}} \rightarrow A \rightarrow \changes{B}$ by:
  $$ (g \cminus_\rightarrow f)(a) \defeq g(a) \cminus f(a) $$
  It's easy to check that this is indeed a minus operator and, therefore, $\cstr{B}^{\cstr{A}}$
  is complete. Thus, trivially, any function $f : C \times A \rightarrow B$ is differentiable
  if and only if its curried version $\Lambda(f) : C \rightarrow B^A$ is.
\end{proof}
