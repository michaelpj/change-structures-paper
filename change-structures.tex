...
homeomorphism in $\cat{SAct}$ (see \cite{kilp2000monoids}) we can see that the structure is
...
expressions. In particular Griffin, Libkin, and Trickey \cite{griffin1997improved} following
Qian and Wiederhold \cite{qian1991incremental} reveal the essential insight that it is necessary to
track both an ``upwards'' and a ``downwards'' difference, and produce a set of
rules that look quite similar to those we derive in \cref{theorem:concreteDatalog}.

Where our presentation improves over Griffin, Libkin, and Trickey is mainly in
...
\cref{theorem:concreteDatalog}. Since bag semantics also has a complete change
...
Several approaches
\cite{gupta1993maintaining,harrison1992maintenance}, most notably DReD
