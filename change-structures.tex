...
continuous change actions, then it becomes a true derivative.
...
\begin{prop}[Derivatives of iteration maps]
  ...
  \begin{align*}
    \derive{\iter_n} &: (A \rightarrow A) \rightarrow \changes{(A \rightarrow A)} \rightarrow \changes{A}\\
...
\end{thm}
\begin{proof}
  We can write our recursive definition of $\derive{\iter}$ as a recurrence relation:
  \begin{align*}
  ...
  $\nextiter_{f, \change{f}}$ is continuous if $\derive{\ev}$ is.

  From the definition of $\derive{\iter_n}$, it follows immediately that whenever $\derive{\ev}$ 
  is continuous, then the set $\{ \derive{\iter}_n \}$ is directed. Hence:
  \begin{itemize}
    \item[ ]
...
The requirements of this theorem are somewhat constraining, but are easily shown
to be true for Boolean algebras with $\cstr{L}_\superpose$ (CHECK THIS CAREFULLY).
