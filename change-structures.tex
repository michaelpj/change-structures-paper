...
\begin{prop}
  Let $f: A \rightarrow B$ be monotone and differentiable. Then $f$ has a
  monotone derivative $\monotoneDerive{f}$.
\end{prop}

\begin{prop}
  Let $f: A \rightarrow B$ be a function, $\change{f} \in \changes{(A
    \rightarrow B)}$, and suppose that $f \cplus \change{f}$ is differentiable.
  Then $\derive{\ev}$ has a derivative $\monotoneDerive{\ev}$ which is monotone
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \fixpoint_A(f) \cplus \derive{\ev}((f, \fixpoint_A(f)), (\change{f}, D(f, \change{f}))
  \end{align*}

  That is, $\derive{\ev}((f \fixpoint_A(f)), (\change{f}, D(f \change{f})))$ is also a
  derivative of $\fixpoint_X$.

  Now, let $D_{max}$ be a maximal derivative of $\fixpoint_x$ such that $D_{max}
  < \derive{\fixpoint_x}$.

  Then $D_2 \defeq \derive{\ev}((f, \fixpoint_A(f)), (\change{f}, D(f, \change{f})))$ is
  \begin{itemize}
    \item A derivative of $\fixpoint_X$
    \item Strictly greater than $D(f, \change{f})$
  \end{itemize}

  Since $D$ is maximal amongst derivatives which are less than
  $\derive{\fixpoint_X}$, it must be that $\derive{\fixpoint_X} \leq D_2$.

  Hence 
  \begin{itemize}
    \item[ ]
      $\fixpoint_A(f \cplus \change{f})$
    \item[$\leq$]\{ $\fixpoint_A$ computes least fixpoints \}\\
      $\fixpoint_A(f) \cplus \derive{\fixpoint_A}(f, \change{f})$
    \item[$\leq$]\{ monotonicity of $\cplus$ \}\\
      $\fixpoint_A(f) \cplus D_2(f, \change{f})$
    \item[=]\{ $D_2$ is a derivative \}\\
      $\fixpoint_A(f \cplus \change{f})$
  \end{itemize}
