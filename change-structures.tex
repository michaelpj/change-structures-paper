...
\begin{defn}[Superposition]
  ...
  where $X^\ast$ is the set of finite sequences of $X$.
\end{defn}
...
\begin{proof}
  ...
  A_\cpluss)$ is given by $\derive{id_1}(a, \change{a}) \defeq a \cplus
  i_1(\change{a})$, and similarly for $id_2$ on the other side.

  TODO: this is super ugly and unclear.
  ...
  given by $\derive{id}(a, \change{a}) \defeq a ((\cpluss \derive{id_1}) +
  (\cplusss \derive{id_2}))^\ast$.
\end{proof}
...
\begin{prop}
  Let $f: A \rightarrow B$ be differentiable and suppose that $\derive{f}$ is
  continuous. Then $\derive{f}$ is monotone.
\end{prop}
\begin{proof}
  THIS IS TRIVIAL

  Suppose $\change{a} \leq \change{b}$.

  Then
  \begin{itemize}
  \item[ ]
    $\change{a} \leq (\change{a} \vee \change{b})$
  \item[$\Rightarrow$]
    $\derive{f}(a, \change{a}) \leq \derive{f}(a, \change{a} \vee \change{b})$
  \item[$\Rightarrow$]
    $\derive{f}(a, \change{a}) \leq \derive{f}(a, \change{a} \vee \change{b})$
  \item[$\Rightarrow$]\{ by continuity \}
    $
    \derive{f}(a, \change{a}) \leq
    (
      \derive{f}(a, \change{a})
      \vee
      \derive{f}(a, \change{b})
    ...
    $
  \item[$\Rightarrow$]
    $\derive{f}(a, \change{a}) \leq \derive{f}(a, \change{b})$
  \end{itemize}
\end{proof}
...
\label{thm:leastFixpointDiff}
  ...
  \begin{itemize}
    ...
    \item $\cplus$ be monotone with respect to the order on $\changes{A}$
    \item $f: A \rightarrow A$ be a differentiable function
    \item $\change{f} \in \changes{(A \rightarrow A)}$ be a change to $f$
...
\label{prop:continuity}
  ...
  complete join-semilattices. Then the following are equivalent:
  \begin{itemize}
    \item $f$ is monotone
    \item $f$ is \textit{continuous} under the Alexandrov topologies on $A$ and $B$
    \item for any $A' \subseteq A$, $$f(\bigvee_{A'} a) = \bigvee_{A'} f(a)$$
  \end{itemize}
\end{prop}
...
\label{prop:supDerivatives}
  ...
  $\changes{B}$ be complete join-semilattices, and suppose that $\cpluss$ is monotone
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[=]\{ monotonicity of $\cpluss$, \ref{prop:continuity} \}\\
