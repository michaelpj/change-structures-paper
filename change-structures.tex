...
This intuition that a change may do too much can be captured in an elegant way by the following
definition:
\begin{defn}[Kernel of a change]
  Let $\cstr{A}$ be a change action and $\change{a} : \changes{A}$. Then the
  \emph{kernel} of $\change{a}$ is the equivalence relation $\kernel_{\change{a}}
...
\begin{defn}[Change order]
  $\change{a} \changeOrder \change{b}$ iff for all $a : A$ it is the case that
