...
\begin{thm}[The Chain Rule]
  ...
  $$\derive{(g \circ f)}(x, \change{x}) = \derive{g}(f(x), \derive{f}(x, \change{x}))$$
  or in curried form 
...
\begin{prop}[Chain rule for sub-derivatives]
  ...
  sub-differentiable functions with sub-derivatives $f', g'$ respectively. Then the composite
  $$ \lambda a. \lambda da. g'(f(a), f'(a, da)) $$
  is a subderivative of $g \circ f$
\end{prop}
...
\begin{proof}
  ...
  \change{f})$ is a fixpoint of $f \cplus \change{f}$.
  \begin{itemize}
  \item[ ]
    $
