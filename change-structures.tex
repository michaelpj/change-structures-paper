...
\citeauthor{cai2014changes} also introduce a useful notion of
\textit{self-maintainability}. A derivative is self-maintainable if it does not
make use of its first argument, and so only depends on the change to the
argument. This makes the derivatives significantly easier to compute. In the
Datalog context, this corresponds to the rule being \textit{linear}, and leads
to much simpler derivatives. The
``independence'' property that self-maintainability provides seems likely to be
useful for parallelizing incremental computation, and we think this notion
