...
step is construct a good change action for Boolean algebras. Along the way, we
...
\begin{defn}[Immediate consequence operator]
  Given a program $\mathbb{P} = (P_1, \dots, P_m)$, where $P_i$ is a predicate,
  the immediate consequence operator $\consq: \Rel_\Gamma^n \rightarrow \Rel_\Gamma^n$ is defined 
  as follows:
  \begin{displaymath}
    \consq(\semR_1, \dots, \semR_n) 
    = (\denote{P_1}_\Gamma(\semR_1, \dots, \semR_n), \dots, \denote{P_m}_\Gamma(\semR_1, \dots, \semR_n))
  \end{displaymath}
\end{defn}
