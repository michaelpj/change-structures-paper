...
\textit{Scott-continuity}. We will 
...
\begin{defn}[Continuous change actions]
  ...
  \begin{itemize}
    ...
    \item $\bot \cplus \bot = \bot$
  \end{itemize}
\end{defn}

We then have the following lemma (which is just a re-statement of a well-known
...
\end{corollary}
\begin{proof}
  It suffices to apply \cref{prop:distributivityLimit} to the directed families $\{ f_i(a) \}$ and 
  $\{ \derive{f_i}(a, \change{a}) \}$.
\end{proof}

We also state the following additional corollary, which will be useful later:
...
TODO: work on flow, move some of this into a proof section
...
In order to do this, we will restrict ourselves to directed complete partial
orders and continuous change actions.

Consider now a continuous function $f : \cstr{A} \rightarrow \cstr{A}$, where $\cstr{A}$ is a continuous change action. Since $A$
is a dcpo, a least fixpoint is guaranteed to exist and can be defined as:
\begin{align*} 
...
as:
  $$\derive{\lfp} \defeq \sqcup_{n \in \NN} \{ \derive{\iter_n} \}$$
provided we can find a derivative of each iteration map $\iter'_n$ such that the resulting 
set $\{ \derive{\iter_n} \}$ is directed.

\begin{prop}[Derivatives of iteration maps]
  The maps $\iter_n$ sending a (differentiable) function $f : \cstr{A} \rightarrow \cstr{A}$ to the element
  $f^n(a)$ are differentiable and one of their derivatives can be defined inductively by:
  \begin{align*}
    \derive{\iter_0} (f, \change{f}) &\defeq \bot\\
    \derive{\iter_{n+1}} (f, \change{f}) &\defeq \derive{\ev}((f, \iter_n(f)), (\change{f}, \derive{\iter_n}(f, \change{f})))
  \end{align*}
  Or, equivalently:
  \begin{align*}
    \nextiter (a, \change{a}) &\defeq (f(a), \derive{\ev}((f, a), (\change{f}, \change{a})))\\
    \iter'_{n+1}(f, \change{f}) &\defeq \pi_2 (\nextiter(\iter_n(f), \iter'_n(f, \change{f})))
  \end{align*}
\end{prop}

From the definition of $\derive{\iter}$, it follows immediately that whenever $\derive{\ev}$ 
is monotonic, then the set $\{ \derive{\iter}_i \}$ is directed. If they are
continuous, its least upper bound can furthermore be computed simply as:
\begin{align*}
  \sqcup_{i \in \NN} \{ \derive{\iter_i} \} &= \pi_2 (\lfp(\nextiter))\\
            &= \lfp(\lambda\ \change{a}. \derive{\ev}((f, \lfp f), (\change{f}, \change{a})))\\
            &= \lfp(\theadjustment(f, \change{f}))\\
            &= \derive{\lfp}(f, \change{f})
\end{align*}

(using \ref{prop:factoringFixpoints})
...
\begin{thm}[Derivatives of least fixpoint operators]
  ...
  \begin{itemize}
    ...
    \item $\derive{\ev}$ be a monotone derivative of the evaluation map
  \end{itemize}
