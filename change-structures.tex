...
\begin{abstract}
  ...
  \emph{derivative} of a program. The derivative allows updating the output
  of a program based on a change to its input.

  We generalise the notion of derivative using \emph{change actions}, and study their algebraic
  properties. We develop the theory of change actions for a variety of common structures
  in computer science, including directed-complete partial orders and Boolean algebras.

  We then show how to compute derivatives of fixpoints, allowing us to give
  generic and compositional account of incremental \emph{evaluation} of Datalog,
  as well as incremental \emph{update} of evaluated Datalog programs.
\end{abstract}
...
a faithful functor $Reach : \cat{CAct} \rightarrow \cat{PreOrd}$ that acts as the identity
on morphisms.
...
$\cstr{A}_\leq$. This gives rise to a (full and faithful) functor $\{\_\}_\leq : \cat{PreOrd} \rightarrow \cat{CAct}$
...
this gives us a proof of the existence of limits, colimits, and exponentials in $\cat{CAct}$.
...
\label{prop:exponentials}
  ...
  \todomario{We can, but I thought it might be more clear to write it explicitly - feel free
    to change it}
  \todomario{Also, I was a bit wrong with this, they aren't monotone functions, they are
    increasing}
  \todomario{Interestingly, the function $\cplus \change{a} : A \rightarrow A$ is
    always increasing, but only monotone if the change structure has a commutator, and this
    is why the previous proof failed.}
...
\begin{proof}
  ...
  \todomario{My bad, it was there from a previous version of the proof}
...
\todompj{Cut?}
\todomario{Actually follows from the equivalence to $\cat{PreOrd}$ and the
  fact that preorders are equivalent to Alexandroff topologies}
...
attempts to derive only the new facts at each iteration \autocite[][section 13.1]{abiteboul1995foundations}. However, ``semi-naive''
as traditionally presented requires a Datalog-specific proof, and is hard to
...
\begin{proof}
  ...
  \todomario{``Left as an exercise for the reader''}
\end{proof}
\fi
