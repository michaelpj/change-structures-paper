...
action - where $a \cplus \change{a} = a \cplus \change{b}$ implies $\change{a} =
\change{b}$ - has unique derivatives, but many change actions are not thin.
...
Ordered change structures are useful because they let us give a sensible order
to our derivatives.
...
Since $\Rel$ is a Boolean algebra, and so is $\Rel^n$, $\denote{t}$ is a functions between Boolean algebras.
