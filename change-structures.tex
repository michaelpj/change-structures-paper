...
computation and maintenance of Datalog programs, showing how to handle 
...
account of incremental evaluation and maintenance. 
...
Since $\Rel$ is a complete Boolean algebra, and so is $\Rel^n$, $\denote{\_}$ is
a function between complete Boolean algebras.

\begin{defn}[Immediate consequence operator]
  Given a program $\mathcal{P} = (P_1, \dots, P_m)$, the immediate consequence operator $\consq: \Rel^n \rightarrow \Rel^n$ is defined as follows:
  \begin{displaymath}
...
action for boolean algebras gives $\changes{\Rel} = \left( \Rel, \Rel \right)$, it
is possible and indeed very natural to compute the two components via a pair of
Datalog formulae, which allows us to reuse our existing Datalog formula
evaluator.\footnote{
...
We will present the derivative as a pair of mutually recursive functions,
$\updiff$ and $\downdiff$, which turns a formula $T$ into new formulae that compute
...
We can now give a derivative for our $treeP$ program (assuming there to be no
changes to $p$ and $child$):
\begin{align*}
...
Again, we know how to do this (\autocite{gupta1993maintaining} and successors),
...
These are easy conditions to satisfy. The former condition is part of the definition of a
new construct, and because we are using the complete change action
$\cstr{L}_\superpose$, the latter condition can \emph{always} be satisfied by using the maximal or
