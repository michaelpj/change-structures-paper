...
Now we can give the proof of \ref{prop:exponentials}.
...
set has a monoid structure: $\cstruct{A}{(\changes{A}_\cplus \times
...
\begin{proof}
  ...
  be monoidal here, so that we can lift $\change{b}$ to $(\mzero, \change{b})$ in
  the superposition.
...
any monoid action, not just change structures.

MARIO: It's definitely true, although the proof may depend (as usual) on the axiom of choice.
I think you can state it more simply by saying that there is a full and faithful functor from
change actions (with a zero) into preorders, but I'm not sure what it buys us. Re. semigroup actions,
if I recall correctly usually one requires that maps be semigroup homomorphisms, which we don't, 
so this is actually not true.

MARIO: Also, there's an equivalence of categories between CStruct and PreOrd, which actually proves
the existence of all limits and colimits in CStruct, but not in a ``convenient'' form.
\begin{corollary}
...
Monotone functions have some useful properties.

\begin{prop}
  Let $f: A \rightarrow B$ be differentiable and monotone. Then there exists a
  derivative $\monotoneDerive{f}$ which is montone.
\end{prop}
\begin{proof}
  Let $D$ be the set of derivatives of $f$. Construct a derivative
  $\monotoneDerive{f}$ as follows: for every $a$, construct the equivalence
  classes of $\change{a}$s such that they have the same $a \cplus \change{a}$.
  For each such class, pick a representative element $\change{a}'$. Then for all
  $\change{a}$ in that class,

  Now suppose $\change{a} \changeOrder \change{b}$. Then
  \begin{itemize}
    \item[ ]$\change{a} \changeOrder \change{b}$
    \item[$\Rightarrow$]\{ definition of $\changeOrder$ \}\\
      $a \cplus \change{a} \leq a \cplus \change{b}$
    \item[$\Rightarrow$]\{ $f$ is monotone \}\\
      $f(a \cplus \change{a}) \leq f(a \cplus \change{b})\}$
    \item[$\Rightarrow$]\{ $\monotoneDerive{f}$ is a derivative \}\\
      $f(a) \cplus \monotoneDerive{f}(a, \change{a}) \leq f(a) \cplus \monotoneDerive{f}(a, \change{b})$
  \end{itemize}

  Then there are three cases:

  If $\monotoneDerive{f}(a, \change{a}) \changeOrder \monotoneDerive{f}(a,
  \change{b})$ then we are done.

  If $\monotoneDerive{f}(a, \change{a}) >_\Delta \monotoneDerive{f}(a,
  \change{b})$, then we have a contradiction to the above equation.

  If $\monotoneDerive{f}(a, \change{a})$ 
...
This is not enough give us a true derivative, however, because we are merely
...
\label{thm:leastFixpointDiff}
  ...
  \begin{itemize}
    ...
    \item $\cplus$ be continuous and monotone with respect to the order on $\changes{A}$
    \item $f: A \rightarrow A$ be a differentiable function
    \item $\change{f} \in \changes{(A \rightarrow A)}$ be a change to $f$
...
First we need a couple of lemmas, including a tiny bit of topology.

\begin{prop}
\label{prop:continuity}
  Let $f: A \rightarrow B$ be a function, and $A$ and $B$ be a
  join-semilattices. Then if $f$ is \textit{continuous} under the Alexandrov
  topologies on $A$ and $B$ iff, for any $A' \subseteq A$, 
...
\label{prop:supDerivatives}
  ...
  $\changes{B}$ be complete lattices, and suppose that $\cpluss$ is continuous
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[=]\{ continuity of $\cpluss$, \ref{prop:continuity} \}\\
...
Now we can give the proof of \ref{thm:leastFixpointDiff}:
...
The requirements of this theorem may seem onerous. However, the setting in which
...
\begin{prop}
  ...
  \begin{itemize}
    ...
    \item $\change{f} \in \changes{(A \rightarrow A)}$ be a
      change to $f$
    \item$f \cplus \change{f}$ be differentiable and monotone
  \end{itemize}
...
\begin{proof}
  ...
  is monotone since $f \cplus \change{f}$ (and hence $\derive{(f \cplus
    \change{f})}$, by REF)and $\cplus$ are monotone.
\end{proof}
