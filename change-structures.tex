...
\begin{proof}
  ...
  $\deriveMonotone{f}$ as follows: for every $a$, construct the equivalence
  ...
  \begin{itemize}
    ...
    \item[$\Rightarrow$]\{ $\deriveMonotone{f}$ is a derivative \}\\
      $f(a) \cplus \deriveMonotone{f}(a, \change{a}) \leq f(a) \cplus \deriveMonotone{f}(a, \change{b})$
  \end{itemize}

  Then there are three cases:

  If $\deriveMonotone{f}(a, \change{a}) \changeOrder \deriveMonotone{f}(a,
  \change{b})$ then we are done.

  If $\deriveMonotone{f}(a, \change{a}) \gt_\Delta \deriveMonotone{f}(a,
  \change{b})$, then we have a contradiction to the above equation.

  If $\deriveMonoton{f}(a, \change{a})$ 
...
\begin{prop}
  ...
  \begin{itemize}
    ...
    \item $\change{f} \in \changes{(A \righarrow A)}$ be a
      change to $f$
