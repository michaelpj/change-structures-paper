...
\begin{itemize}
  ...
  \item A description of the properties of change structures over posets
    (\ref{sec:order}), including theobrems that characterize the range of
...
\label{thm:fixpointDiff}
  ...
  \end{itemize}
  then the previous expression is also a least fixpoint and $\derive{\fixpoint_A}$ is a derivative.
\end{thm}
...
\begin{proof}
  ...
  MPJ: possibly this should be its own lemma? It's a similar to form to Cai's
  ``Incrementalization'' lemma, and we could drop the final rolling of the
  fixpoint for a simpler statement

  First, we prove an equivalence that we will use in both parts. For any $\change{a} \in \changes{A}$
  \begin{itemize}
  \item[ ]
    $
    (f \cplus \change{f})(\thefixpoint \cplus \change{a})
    $
  \item[=]\{ definition of $\cplus$ for $\changes{(A \rightarrow A)}$ \}\\
    $
    f(\thefixpoint \cplus \change{a})
    \cplus
    \change{f}(\thefixpoint \cplus \change{a})
    $
  \item[=]\{ derivative property of $\derive{f}$ \}\\
    $
    f(\thefixpoint)
    \cplus
    \derive{f}(\thefixpoint, \change{a})
    \cplus
    \change{f}(\thefixpoint \cplus \change{a})
    $
  \item[=]\{ semigroup action property \}\\
    ...
    \change{f}(\thefixpoint \cplus \change{a})
    $
  \item[=]\{ definition of $\theadjustment$, rolling the fixpoint \}\\
    $
    \thefixpoint
    \cplus
    \theadjustment(\thefixpoint, f, \change{f})(\change{a})
    $
  \end{itemize}

  For the first part, we set $\change{a} = \derive{\fixpoint_A}(f, \delta{f})$
  \begin{itemize}
  \item[ ]
    $
    (f \cplus \change{f})(\thefixpoint \cplus \derive{\fixpoint_A}(f, \delta{f}))
    $
  \item[=]\{ previous equivalence \}\\
    $
    \thefixpoint
    \cplus
    \theadjustment(\thefixpoint, f, \change{f})(\derive{\fixpoint_A}(f, \delta{f}))
    $
  \item[=]\{ definition of $\derive{\fixpoint_A}$ \}\\
    $
    \thefixpoint
    \cplus
    \theadjustment(\thefixpoint, f, \change{f})(\fixpoint_{\changes{A}}(\theadjustment(\thefixpoint, f, \change{f})))
    $
  \item[=]\{ rolling the fixpoint \}\\
    $
    \thefixpoint
    \cplus
    \fixpoint_{\changes{A}}(\theadjustment(\thefixpoint, f, \change{f}))
    $
  \item[=]\{ definition of $\derive{\fixpoint_A}$ \}\\
    $
    \thefixpoint
  ...
  Then suppose that $D$ is a derivative of $\fixpoint_X$. Then $\thefixpoint \cplus D(f,
  \change{f})$ is a fixpoint of of $f \cplus \change{f}$.
  \begin{itemize}
  \item[ ]
    $
    (f \cplus \change{f})(\thefixpoint \cplus D(f, \change{f}))
    $
  \item[=]\{ previous equivalence \}\\
    $
    \thefixpoint
    \cplus
    \theadjustment(\thefixpoint, f, \change{f})(D(f \change{f}))
    $
  \item[$\geq$]\{ STILL WRONG NEED TO SHOW OTHER THING IS PREFIX POINT monotonicity of $\cplus$, $\fixpoint_{\changes{A}}$ computes a
    least prefix point \}\\
    $
    \thefixpoint
    \cplus
  ...
  Therefore $\thefixpoint \cplus \derive{\fixpoint_A}(f, \change{f})$ is a least fixpoint of $f
  \cplus \change{f}$, and hence $\derive{\fixpoint_A}$ is a derivative.
\end{proof}
...
specifics of the evaluation engine. However, we give an example
here.\footnote{This is the variant used at Semmle.}
...
The cases for $\exists$ look quite different for $\updiff$ and $\downdiff$, but
...
We can easily extend this to a derivative for $\consq$.
